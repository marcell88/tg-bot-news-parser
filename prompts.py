...
1. **ТЭГ 1** (О чем?) → напиши основную тему даной новости (одним словом или одним законченным словосочетанием)

2. **ТЭГ 2** (Кто?) → перечисли 3-5 основных уникальных действующих лиц (людей, организаций, стран).
- имена собственные в приоритете, если есть три или более собственных имени, то ими и ограничься.
...
3. **ТЭГ 3** (Что происходит?) → опиши ключевой процесс/событие, которое произошло или происходит (одним словом или одним законченным словосочетанием)

4. ТЭГ 4 (Где?) → укажи 1-3 ключевых географических объекта (города, регионы, страны), где происходит основное событие.
- Указывай локации от наиболее конкретной к общей (например: Москва, Россия).
- Используй только общепринятые краткие названия.
- Если событие глобальное и не привязано к месту, укажи "Глобальное".
- Если событие не глобальное, но к месту не привязано укажи "Нет локации"

5. ТЭГ 5 (Последствия/Контекст?) → определи ключевое следствие, цель или причину события (одним словом или словосочетанием).
- Этот тег должен отвечать на вопрос "К чему это привело?" или "Ради чего это делается?" или "Почему это произошло?".
- Примеры: санкции, расследование, технологический прорыв, коалиционное соглашение, обвал рынка.

**Правила:**
- Попробуй уточнить детали из контекста (например, "местные СМИ" но новость про Индию, так что можно уточнить "СМИ Индии")
- Фокус на том, что отличает ЭТУ новость
- Используй только текст новости не домысливай. Если инфрмации нет - просто верни пустую строку.

## ФОРМАТ ОТВЕТА
```json
{
  "tag1": "Беспилотный транспорт",
  "tag2": "Москва, СтарТек, Минтранс РФ",
  "tag3": "Испытания беспилотного такси", 
  "tag4": "Москва, Садовое кольцо, Россия",
  "tag5": "Снижение транспортной загрузки"
}
...
TAGED_SCHEMA: Dict[str, Any] = {
    ...
    "properties": {
        ...
        "tag5": {
            "type": "string",
            "description": "Последствия/Контекст?"
        }
    },
    "required": [
        "tag1",
