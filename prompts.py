...
FINAL_SCORE_THRESHOLD = 8.0

TAGED_PROMPT = """

Проанализируй текст и создай 5 уникальных тегов, отвечающих на ключевые вопросы:

1. **СУБЪЕКТ** (Кто? Что?) → перечисли всех основных уникальных действующих лиц (не более 3х, в алфавитном порядке), уникальные и собственные имена в приоритете.
2. **ДЕЙСТВИЕ** (Что происходит?) → опиши ключевой процесс/событие что произошло или происходит
3. **МЕСТО И ВРЕМЯ** (Как?) → уникальное место или время
4. **ПРИЧИНА И КОНТЕКСТ** (Зачем?) → уникальная причина или контекст происходящего
5. **ИСТОЧНИК** (Для кого?) → уникальный источник информации

**Правила:**
- Каждый тег: 1-3 слова
- Имена собственные в ПРИОРИТЕТЕ (названия, имена, бренды)
- Если несколько имен собственных - включи все значимые, по алфавиту
- Максимальная уникальность и специфичность
- Избегать общих слов ("люди", "проблема")
...
```json
{
  "subject": "Илья Варламов",
  "action": "Внесение в список экстремистов",
  "time_place": "Нарушение закона об иноагентах", 
  "reason": "Уголовное преследование",
...
TAGED_SCHEMA: Dict[str, Any] = {
    ...
    "properties": {
        "subject": {
            "type": "string",
            "description": "Самые характерные лица/организации (1-3 слова, имена собственные в приоритете)"
        },
        "action": {
            "type": "string", 
            "description": "Ключевой процесс или событие (1-3 слова, максимально специфичное)"
        },
        "time_place": {
            "type": "string",
            "description": "Уникальный способ реализации (1-3 слова, описывает механизм)"
        },
        "reason": {
            "type": "string",
            "description": "Конечная цель или мотивация (1-3 слова, конкретный результат)"
        },
        "source": {
            "type": "string",
            "description": "Специфическая целевая группа (1-3 слова, узкая аудитория)"
        }
    },
    "required": [
        "subject",
