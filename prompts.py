...
2. Если основная тема и смысл сообщения - рекламные и маркетинговые материалы, откровенный пиар товаров, услуг, персон, включая аннонсы фильмов, спектаклей, игр (просто призыв подписаться или перейти по ссылке не является рекламой).
...
## ИНСТРУКЦИЯ
Ты — редактор с чутьем на сенсации. Прочитай сообщение ниже и переработай его в единый, мощный микротекст (2-3 предложения).

Алгоритм действий:
1) Найди один яркий образ. Проанализируй суть новости и выдели главный конфликт или противоречие (старое/новое, большое/маленькое, живое/мертвое). Создай метафору, которая отражает именно этот конфликт, а не пересказывает детали.
2) Первое предложение — чистая метафора. Опиши этот образ максимально сжато и зрелищно. Запрещено: использовать обороты-паразиты вроде «это как будто», «словно», «точно», «это случай, когда». Метафора должна быть самодостаточной и утвердительной.
3) Оставшаяся часть — суть новости. В следующих 1-2 предложениях четко и информативно изложи, что произошло на самом деле. Ты можешь обыграть первоначальный образ (связать с ним факты), но главная цель этой части — донести содержание новости без потери смысла.
4) Собери в абзац. Текст должен быть единым целым, без маркировки структуры. Переход от метафоры к фактам должен быть естественным.

**Важные ограничения**: 
1) макстмальное количество символов - 200 (включая пробелы)
2) избегай прямых обвинений, юридически рискованных формулировок (например, "грабительские", "воровские", "мошеннические") и любых высказываний, которые могут быть истолкованы как клевета или обвинение в преступлении.
3) Также игнорируй часть сообщения с призывом подписаться, перейти по ссылке и проч. Только основная новость должна тебя интересовать.
4) Оставайся на нейтральной позиции, если речь идет о политическом или военном конфликте, какой-либо конкуренции (исключение - всегда занимай сторону России).
5) Постарайся фантазировать по минимуму, используй в основном информацию из оригинального текста.
6) Если сам оригинальный текст короткий - то можно взять его как есть. Избегай излишней иронии и превращения новости в анекдот: метафора должна украшать факты, а не заменять их.

Запрещено: оскорбления, мат, двусмысленности, политические провокации.

## ФОРМАТ ОТВЕТА
Одна строка, состоящая из 2-3 предложений. Первое — метафора без «как будто», остальные — информативная часть.
...
LT_PROMPT = """Ты - эксперт по контентной стратегии и диверсификации тем.

**Задача:**
Оцени, насколько тема и настроение нового сообщения способствует диверсификации контента относительно текущего распределения тем и настроения (LT).

**Критерии оценки от 1 до 10:**
- 1-2: Тема и/или настроение полностью повторяет очень популярные текущие темы
- 3-4: Тема и/или настроение близка к существующим, но с небольшими отличиями  
- 5-6: Тема и/или настроение умеренно новая, частично диверсифицирует контент
- 7-8: Тема и/или настроение достаточно новая, хорошо диверсифицирует контент
- 9-10: Тема и/или настроение полностью новая, отлично диверсифицирует контент, заполняет пробелы

**Учти:**
1. Чем выше вес текущей темы или настроения, тем меньше баллов за повторение
2. Если тема или настроение совершенно новая (нет в текущих категориях) - ставь высокие баллы
3. Учитывай семантическую близость, а не только точное совпадение слов

Верни ТОЛЬКО JSON согласно схеме."""

LT_SCHEMA: Dict[str, Any] = {
    "type": "object",
    "properties": {
        "lt_score": {
            "type": "number",
